...
class Requirement(BaseModel):
    ...
    attendance:int=100     #required attendance for this role (eg remote supervisor=50%)
    geofence:str='_immediate'     #must not clash with duty outside this zone (eg main theatre)
...
class ActivityTemplate(BaseModel):
    ...
    def model_post_init(self,_):
        print('post init, innit?',self.model_extra)
    ...
    def materialise(self,date,force=False):
        ...
        if force or self.ruleset.matches(date):
            return Activity(
                ...
                activity_finish=datetime.datetime.combine(date,self.start_time)+self.duration
            )
...
class StaffAssignment(SQLModel,table=True):
    "staff assignment"
    activity_id:uuid.UUID=SQLField(primary_key=True,foreign_key='activity.activity_id')
...
class Activity(SQLModel,table=True):
    ...
    requirement:RequirementList=SQLField(sa_type=DBRequirementList,default_factory=list)
...
def make_potential_assignments(model:cp_model.CpModel,offers):
    ...
    staff:list[Staff]=[]
    offers=[(model.new_bool_var(''),staff_id,activity_set) for staff_id,activity_set in offers]
    ...
    geofences:dict[tuple[uuid.UUID,str,datetime.datetime,datetime.datetime],tuple[cp_model.IntervalVar,cp_model.IntVar]]={}
                ...
                for staff_member in staff:
                    if requirement.skills.issubset(set(staff_member.skills.split())):
                        req=PotentialStaffAssignment(
                            ...
                            actual_location=activity.location if requirement.geofence=='_immediate' else requirement.geofence,
                        ...
                        while low_bound<activity.activity_finish:
                            ...
                            key=(staff_member.id,req.actual_location,low_bound,high_bound)
                            if key not in geofences:
                                is_active=model.new_bool_var('')
                                geofences[key]=(model.new_optional_fixed_size_interval_var(
                                    low_bound.timestamp(),
                        ...
                        model.add_implication(req_set.is_active==0,req.is_active==0)
                        this_req.append(req)
                        all_potential_assignments.append(req)
                model.add(sum(psa.is_active for psa in this_req)>=requirement.mandatory)
                model.add(sum(psa.is_active for psa in this_req)<=(requirement.mandatory+requirement.optional))
    ...
    by_activity_and_staff:dict[tuple[uuid.UUID,uuid.UUID],list[PotentialStaffAssignment]]={}
    for psa in all_potential_assignments:
        by_activity_and_staff.setdefault((psa.activity_id,psa.staff_id),[]).append(psa)
    offers_by_activity_and_staff:dict[tuple[uuid.UUID,uuid.UUID],list[cp_model.IntVar]]={}
    for is_active,staff_member,activityset in offers:
        for act in activityset:
            offers_by_activity_and_staff.setdefault((act,staff_member.id),[]).append(is_active)
            model.add_at_least_one(psa.is_active for psa in by_activity_and_staff[(act,staff_member.id)]).only_enforce_if(is_active)
    ...
    for psa in all_potential_assignments:
        ...
        intervals_by_staff.setdefault(psa.staff_id,[]).append((psa.attendance,model.new_optional_fixed_size_interval_var(start_time,duration,psa.is_active)))
