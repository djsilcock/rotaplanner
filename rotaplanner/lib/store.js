...
export function dutyReducer(state, action) {
    ...
    switch (dutyType) {
        case 'CLEAR':
                return setDeep(state, [name, shift, day], undefined)
        case 'DEFINITE_ICU':
        case 'ICU':
                return names.reduce((intstate,name2) => (
                    updateDeep(intstate, [name2, shift, day], (val) => {
                        if (name == name2) {
                            return dutyType
                        }
        ...
        default:
            return updateDeep(state,[name,shift,day],(val)=>(val==dutyType?undefined:dutyType))
    }
}
