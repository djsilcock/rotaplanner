(() => {
  ...
  // new_rotaplanner/vendored/datastar.js
  var datastar_exports = {};
  __export(datastar_exports, {
    action: () => k,
    actions: () => Rt,
    attribute: () => p,
    beginBatch: () => M,
  ...
  var St = { camel: (e) => e.replace(/-[a-z]/g, (t) => t[1].toUpperCase()), snake: (e) => e.replace(/-/g, "_"), pascal: (e) => e[0].toUpperCase() + St.camel(e.slice(1)) };
  var L = (e, t, n = "camel") => {
...
Context: ${i}`, r;
  ...
  var Rt = new Proxy({}, { get: (e, t) => ye.get(t)?.apply, has: (e, t) => ye.has(t), ownKeys: () => Reflect.ownKeys(ye), set: () => false, deleteProperty: () => false });
  ...
  document.addEventListener(q, (e) => {
    ...
    t && t.apply({ error: he.bind(0, { plugin: { type: "watcher", name: t.name }, element: { id: e.target.id, tag: e.target.tagName } }) }, e.detail.argsRaw);
  });
  var ve = (e) => {
    At.set(e.name, e);
  };
  ...
  var xt = (e, t, n, r) => {
    {
      ...
      if ((!r || Ze.has(o)) && c) {
        ...
        if (f) {
          let v;
          l.rx = (...A) => (v || (v = rn(n, { returnsValue: c.returnsValue, argNames: c.argNames, cleanups: m })), v(e, ...A));
        }
        let y = c.apply(l);
        y && m.set("attribute", y);
        let b = be.get(e);
...
`);
    ...
    try {
      ...
      return (l, ...u) => {
        let d = (h, f, ...m) => {
          let y = he.bind(0, { plugin: { type: "action", name: h }, element: { id: l.id, tag: l.tagName }, expression: { fnContent: s, value: e } }), b = Rt[h];
        ...
        try {
          ...
          throw console.error(h), he({ element: { id: l.id, tag: l.tagName }, expression: { fnContent: s, value: e }, error: h.message }, "ExecuteExpression");
        }
      };
    } catch (c) {
      throw console.error(c), he({ expression: { fnContent: s, value: e }, error: c.message }, "GenerateExpression");
    }
  };
  k({ name: "peek", apply(e, t) {
    F();
    try {
      return t();
    } finally {
  ...
  k({ name: "setAll", apply(e, t, n) {
    F();
    let r = _(n);
    Y(r, () => t), O(r), P();
  } });
  k({ name: "toggleAll", apply(e, t) {
    F();
    let n = _(t);
    Y(n, (r) => !r), O(n), P();
  } });
  var Ee = (e, t, n = true) => k({ name: e, apply: async ({ el: r, evt: s, error: o, cleanups: i }, a, { selector: c, headers: l, contentType: u = "json", filterSignals: { include: d = /.*/, exclude: h = /(^|\.)_/ } = {}, openWhenHidden: f = n, payload: m, requestCancellation: y = "auto", retry: b = "auto", retryInterval: v = 1e3, retryScaler: A = 2, retryMaxWaitMs: I = 3e4, retryMaxCount: ne = 10 } = {}) => {
    ...
    try {
      ...
      let V = { Accept: "text/event-stream, text/html, application/json", "Datastar-Request": true };
      u === "json" && (V["Content-Type"] = "application/json");
      let Ae = Object.assign({}, V, l), re = { method: t, headers: Ae, openWhenHidden: f, retry: b, retryInterval: v, retryScaler: A, retryMaxWaitMs: I, retryMaxCount: ne, signal: de.signal, onopen: async (g) => {
...
`)) {
        ...
        let N = Object.fromEntries(Object.entries(E).map(([R, w]) => [R, w.join(`
`)]));
        ee(B, r, N);
      }, onerror: (g) => {
  ...
  var ee = (e, t, n) => document.dispatchEvent(new CustomEvent(q, { detail: { type: e, el: t, argsRaw: n } }));
...
${l}` : l;
  ...
  var dn = (e, t, { signal: n, headers: r, onopen: s, onmessage: o, onclose: i, onerror: a, openWhenHidden: c, fetch: l, retry: u = "auto", retryInterval: d = 1e3, retryScaler: h = 2, retryMaxWaitMs: f = 3e4, retryMaxCount: m = 10, responseOverrides: y, ...b }) => new Promise((v, A) => {
    let I = { ...r }, ne, de = () => {
  ...
  p({ name: "attr", requirement: { value: "must" }, returnsValue: true, apply({ el: e, key: t, rx: n }) {
    let r = (a, c) => {
  ...
  var Ft = G("bind");
  p({ name: "bind", requirement: "exclusive", apply({ el: e, key: t, mods: n, value: r, error: s }) {
    ...
    if (e instanceof HTMLInputElement) switch (e.type) {
      ...
      case "file": {
        let f = () => {
          ...
          Promise.all(m.map((b) => new Promise((v) => {
            ...
            A.onload = () => {
              if (typeof A.result != "string") throw s("InvalidFileResultType", { resultType: typeof A.result });
      ...
      if (e.multiple) {
        ...
        i = (m) => [...m.selectedOptions].map((y) => {
          ...
          for (let y of e.options) m.includes(y.value) ? (f.set(y.value, "string"), y.selected = true) : m.includes(+y.value) ? (f.set(y.value, "number"), y.selected = true) : y.selected = false;
        };
      }
  ...
  p({ name: "class", requirement: { value: "must" }, returnsValue: true, apply({ key: e, el: t, mods: n, rx: r }) {
    e &&= L(e, n, "kebab");
    let s, o = () => {
  ...
  p({ name: "computed", requirement: { value: "must" }, returnsValue: true, apply({ key: e, mods: t, rx: n, error: r }) {
    if (e) T([[L(e, t), ke(n)]]);
    else {
  ...
  p({ name: "indicator", requirement: "exclusive", apply({ el: e, key: t, mods: n, value: r }) {
  ...
  p({ name: "init", requirement: { key: "denied", value: "must" }, apply({ rx: e, mods: t }) {
    let n = () => {
      M(), e(), x();
    };
    n = Q(n, t);
  ...
  p({ name: "json-signals", requirement: { key: "denied" }, apply({ el: e, value: t, mods: n }) {
  ...
  p({ name: "on", requirement: "must", argNames: ["evt"], apply({ el: e, key: t, mods: n, rx: r }) {
    let s = e;
  ...
  p({ name: "on-intersect", requirement: { key: "denied", value: "must" }, apply({ el: e, mods: t, rx: n }) {
    let r = () => {
      M(), n(), x();
    };
    r = Q(r, t), r = ce(r, t);
  ...
  p({ name: "on-interval", requirement: { key: "denied", value: "must" }, apply({ mods: e, rx: t }) {
    let n = () => {
      M(), t(), x();
    };
    n = Q(n, e);
  ...
  p({ name: "on-signal-patch", requirement: { value: "must" }, argNames: ["patch"], returnsValue: true, apply({ el: e, key: t, mods: n, rx: r, error: s }) {
  ...
  p({ name: "ref", requirement: "exclusive", apply({ el: e, key: t, mods: n, value: r }) {
    let s = t != null ? L(t, n) : r;
    T([[s, e]]);
  } });
  var kt = "none";
  var Ht = "display";
  p({ name: "show", requirement: { key: "denied", value: "must" }, returnsValue: true, apply({ el: e, rx: t }) {
    let n = () => {
  ...
  p({ name: "signals", returnsValue: true, apply({ key: e, mods: t, rx: n }) {
  ...
  p({ name: "style", requirement: { value: "must" }, returnsValue: true, apply({ key: e, el: t, rx: n }) {
  ...
  p({ name: "text", requirement: { key: "denied", value: "must" }, returnsValue: true, apply({ el: e, rx: t }) {
    let n = () => {
  ...
  var pn = ["remove", "outer", "inner", "replace", "prepend", "append", "before", "after"];
  var gn = ["html", "svg", "mathml"];
  ve({ name: "datastar-patch-elements", apply(e, { selector: t = "", mode: n = "outer", namespace: r = "html", useViewTransition: s = "", elements: o = "" }) {
    ...
    let i = { selector: t, mode: n, namespace: r, useViewTransition: s.trim() === "true", elements: o };
  ...
  var rt = (e, t) => {
    ...
    if (n === 1) {
      ...
      o && !r.hasAttribute("data-scope-children") && r.setAttribute("data-scope-children", ""), r.isEqualNode(s) || Gt(r, s), o && r.dispatchEvent(new CustomEvent("datastar:scope-children", { bubbles: false }));
    }
  ...
  ve({ name: "datastar-patch-signals", apply({ error: e }, { signals: t, onlyIfMissing: n }) {
    if (t) {
      let r = n?.trim() === "true";
      O(ae(t), { ifMissing: r });
  ...
  dragdrop_default(datastar_exports);
})();
