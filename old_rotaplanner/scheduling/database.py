...
from old_rotaplanner.database import db
...
def get_assigned_activities(start_date, end_date):
    ...
    for activity, staff in result:
        activities.setdefault((activity.activity_start.date(), staff), set()).add(
            activity
        )
        activities.setdefault((activity.activity_start.date(), None), set()).add(
            activity
        )
    return activities
...
def get_activities_for_date(date):
    ...
    return db.session.execute(
        select(Activity).where(
            (Activity.activity_start >= start_date)
            & (Activity.activity_start <= finish_date)
        )
    ).scalars()
...
def unassign_activity(staff, activity_id):
    db.session.execute(
        delete(StaffActivities).where(
            (StaffActivities.activity_id == activity_id)
            & (StaffActivities.staff_id == staff)
        )
    )
    db.session.commit()
...
def find_overlaps(staff, activity_id):
    ...
    result = db.session.execute(
        ...
        .where(StaffActivities.staff_id == staff)
    ).scalars()
    return list(result)
...
def generate_offers_for_range(start, finish):
        ...
        for date in activities_by_date:
            ...
            for index, offer_combination in enumerate(offers_for_day):
                ...
                for activity in offer_combination:
                    activities_by_offer.setdefault(
                        (activity, supply_template.staff), set()
                    ).add((supply_template.id, date, index))
    ...
    for o, v in offers.items():
        model.add_all_true(activities_active[act] for act in v).only_enforce_if(
            offers_active[o]
        )
    for activity, v in activities_by_offer.items():
        model.at_least_one(offers_active[offer] for offer in v).only_enforce_if(
            activities_active[activity]
        )
